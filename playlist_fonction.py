...
def recup_song():
    ...
    file_list = os.listdir("Song")
    conn = sqltor.connect('Ctk/database.db')
    cursor = conn.cursor()
    ...
    for fichier in range(len(file_list)):
        ...
        file_list[fichier] = 'Song/' + file_list[fichier]
        playlist.append(file_list[fichier])
        donnees_liste.append((file_list[fichier], j, duration))
        print(donnees_liste)

    cursor.executemany('INSERT INTO Songs (tile,vote,duration) VALUES (?, ?, ?)', donnees_liste)
    conn.commit()
    conn.close()
    return playlist
